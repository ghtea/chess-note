...
function* moveInQuizPlaying(action: actions.data.quiz.type__MOVE_IN_QUIZ_PLAYING) {
  ...
  let sanSeries = quizPresent.sanSeries || [];
  ...
  try {
    ...
    if (moveTried === null) {
      ...
      sanSeries = [...sanSeries, moveTried.san];
      // 먼저 플레이어가 둔 수 둔다
      const replacement = {
        ...quizPresent,
      ...
      const correctSanSeriesListIncluding = correctSanSeriesList.filter((sanSeriesCorrectEach) => {
        const indexIncludingAll = sanSeriesCorrectEach.join('-').indexOf(sanSeries.join('-'));
        // 처음 움직임부터 동일해야 한다
      ...
      // 여태까지의 움직임 만족하는게 있다면
      if (correctSanSeriesListIncluding.length > 0) {
        const indexSameMove = correctSanSeriesListIncluding.findIndex(
          (e) => e.length === sanSeries.length,
        );
        ...
        // 움직임이 더 남아 있어서, 상대 움직임을 자동으로 실현해야줘야 한다
          ...
          const sanNext = correctSanSeriesListIncluding[0][sanSeries.length - 1 + 1];
          ...
          if (!result) {
            ...
            sanSeries = [...sanSeries, result.san];
