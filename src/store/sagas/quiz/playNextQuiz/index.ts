...
export default function* playNextQuiz(action: actions.quiz.type__PLAY_NEXT_QUIZ) {
  ...
  console.log('hello next quiz')
  const focusingId: string = yield select((state: StateRoot) => state.quiz.data.focusing.id);

  const playingIdList: string[] = yield select(
    (state: StateRoot) => state.quiz.state.playingIdList,
  );
  ...
  if (currentIndex === -1){
    ...
    const myQuizList: types.quiz.Quiz[] = yield select(
      (state: StateRoot) => state.quiz.data.myQuizList,
    );
    const publicQuizList: types.quiz.Quiz[] = yield select(
      (state: StateRoot) => state.quiz.data.publicQuizList,
    );
